...
import { Play, Pause, RotateCcw, Volume2, VolumeX, Maximize2, Lightbulb, ArrowLeft, Server, Activity, Film } from 'lucide-react';
...
export default function VideoPlayer({
  ...
  const [currentServer, setCurrentServer] = useState<'Server 1' | 'Server 2' | 'Server 3'>('Server 1');
  ...
  useEffect(() => {
    if (isCinemaMode && videoRef.current) {
      onShowNotification('Đã bật chế độ xem rạp (Cinema Mode). Làm tối các chi tiết xung quanh!');
    }
    ...
    if (videoRef.current) {
      if (isPlaying) {
        ...
        videoRef.current.play()
          ...
          .catch(() => {
            onShowNotification('Lỗi phát video. Đang tự động kết nối nguồn mới...');
          });
      }
    }
  };
  ...
  const handleServerChange = (server: 'Server 1' | 'Server 2' | 'Server 3') => {
    ...
    setTimeout(() => {
      ...
      setIsPlaying(true);
      onShowNotification(`Đã chuyển sang ${server}. Tốc độ đường truyền ổn định!`);
    }, 1200);
  };
    ...
    <div id="video-player-root" className="relative space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-4 sm:px-0 gap-4 z-10 relative">
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <span className="text-xs text-slate-400 font-medium">Bạn đang xem trên nguồn phát chính thức</span>
        </div>
      </div>
      ...
      >
        ...
        <div className="relative aspect-video w-full bg-black flex items-center justify-center group/video">
          ...
          {isServerLoading ? (
            <div id="server-shift-spinner" className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950 gap-3 z-30">
              <div className="h-10 w-10 animate-spin rounded-full border-4 border-slate-700 border-t-red-600" />
              <p className="text-xs text-slate-400 font-mono">Đang kết nối tới phân vùng {currentServer}...</p>
            </div>
          ) : null}
        ...
        <div className="p-4 sm:p-6 border-t border-slate-900 group bg-slate-950/40 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-slate-400 font-bold">Nguồn {currentServer}</span>
            </div>
            <h1 className="text-lg sm:text-2xl font-extrabold text-white">{movie.title}</h1>
            <p className="text-xs text-slate-500 font-mono">{movie.originalTitle} ({movie.year})</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              <Lightbulb className={`h-4 w-4 ${isCinemaMode ? 'fill-current animate-bounce' : ''}`} />
      ...
      <div id="servers-list-panel" className="rounded-xl border border-slate-800/80 bg-slate-900/60 p-4 sm:p-5 space-y-3 z-10 relative">
        ...
        <p className="text-xs text-zinc-500 leading-normal">
          Hãy chuyển đổi server phát dự phòng khác dưới đây nếu bạn cảm thấy đường truyền không ổn định, phim bị đứng khung hình hoặc lỗi kết nối ban đầu.
        </p>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 [&>*]:cursor-pointer">
          {[
            { id: 'Server 1', label: 'Server 1 (Tốc Độ Cao - Mặc Định)', note: 'Đường truyền Viettel, FPT mượt mà' },
            { id: 'Server 2', label: 'Server 2 (Sao Lưu - Full HD)', note: 'Bản nén dung lượng vừa, ít giật lag' },
            { id: 'Server 3', label: 'Server 3 (Server Dự Phòng - chất lượng gốc)', note: 'Chất lượng gốc độ phân giải cao' }
          ].map((srv) => (
            <div
              key={srv.id}
            ...
            >
              <span className={`text-xs font-bold leading-normal ${currentServer === srv.id ? 'text-red-400' : 'text-slate-200'}`}>
                {srv.label}
              </span>
