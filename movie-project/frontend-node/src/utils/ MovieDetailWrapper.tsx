...
const MovieDetailWrapper = ({movies, watchlistIds, handlePlayClick, handleToggleWatchlist, handleMovieClick, showNotification}:
  {movies:Movie[], watchlistIds:string[], handlePlayClick:(movieId: string) => void, handleToggleWatchlist:(id: string) => void, handleMovieClick:(movieId: string) => void, showNotification:(message: string) => void}) => {
      ...
      async function load() {
        ...
        try {
          ...
          if (!localMovie) {
            ...
            localMovie =
              cachedMovies.find((m) => m.id === id) ?? null;
          }
